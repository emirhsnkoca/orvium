...
import { defaultConfig } from './GithubConfig.js';
...
export class GithubModule extends BaseModule {
    ...
    async startModule() {
        ...
        this.moduleManager.addActionHook(this, ModuleHookAction.ClientConfig, 1, "github login config", async (clientConfig) => {
        ...
        this.moduleManager.addActionHook(this, ModuleHookAction.SessionStart, 6, "Github login check", (session, userInput) => this.processSessionStart(session, userInput));
        this.moduleManager.addActionHook(this, ModuleHookAction.SessionComplete, 5, "Github save session", (session) => this.processSessionComplete(session));
        this.moduleManager.addActionHook(this, ModuleHookAction.SessionRewardFactor, 5, "Github reward factor", (session, rewardFactors) => this.processSessionRewardFactor(session, rewardFactors));
        ServiceManager.GetService(FaucetWebApi).registerApiEndpoint("githubCallback", (req, url, body) => this.processGithubAuthCallback(req, url, body));
    ...
    async processSessionStart(session, userInput) {
        ...
        for (let i = 0; i < this.moduleConfig.checks.length; i++) {
            ...
            if (check.required && passed === false) {
                ...
                throw new FaucetError("GITHUB_CHECK", errMsg);
            }
    ...
    async processGithubAuthCallback(req, url, body) {
        let errorCode;
        let authCode;
        let authResult = {};
        if ((errorCode = url.query['error'])) {
            authResult['errorCode'] = errorCode;
            authResult['errorMessage'] = url.query['error_description'];
        }
        else if ((authCode = url.query['code'])) {
            try {
    ...
    async checkRestriction(githubUserId, restriction) {
        ...
        if (restriction.limitCount > 0 && finishedSessions.length >= restriction.limitCount) {
            ...
            throw new FaucetError("GITHUB_LIMIT", errMsg);
        }
