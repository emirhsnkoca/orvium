...
export class PassportResolver {
    ...
    async verifyUserPassport(addr, passportJson) {
    ...
    getNewestPassportStampTime(passport) {
        let newest = 0;
        if (passport.stamps) {
    ...
    async refreshPassport(addr, passport) {
    ...
    async verifyPassportIntegrity(addr, passport) {
        ...
        await Promise.all(passport.stamps.map(async (stamp) => {
            ...
            if (stamp.credential.proof.verificationMethod.substring(0, stamp.credential.issuer.length) !== stamp.credential.issuer) {
    ...
    savePassportToCache(passport, cacheFile) {
        let trimmedPassport = {
    ...
    async buildPassportInfo(addr, passport) {
        let passportInfo;
        let now = Math.floor((new Date()).getTime() / 1000);
        let savePromises = [];
        if (passport) {
...
//# sourceMappingURL=PassportResolver.js.map