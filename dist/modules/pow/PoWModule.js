...
import { defaultConfig, PoWHashAlgo } from './PoWConfig.js';
...
export class PoWModule extends BaseModule {
    ...
    startModule() {
        ...
        this.moduleManager.addActionHook(this, ModuleHookAction.ClientConfig, 1, "mining config", async (clientConfig) => this.processClientConfig(clientConfig));
        this.moduleManager.addActionHook(this, ModuleHookAction.SessionInfo, 1, "mining state", async (session, moduleState) => this.processSessionInfo(session, moduleState));
        this.moduleManager.addActionHook(this, ModuleHookAction.SessionStart, 10, "mining session", (session) => this.processSessionStart(session));
        this.moduleManager.addActionHook(this, ModuleHookAction.SessionRestore, 10, "mining session restore", (session) => this.processSessionRestore(session));
        this.moduleManager.addActionHook(this, ModuleHookAction.SessionComplete, 10, "kill clients", (session) => this.processSessionComplete(session));
    ...
    async processSessionRestore(session) {
        if (session.getSessionData("skip.modules", []).indexOf(this.moduleName) !== -1)
            return;
        if (session.getSessionStatus() !== FaucetSessionStatus.RUNNING)
            return;
    ...
    async processPoWSessionClose(session) {
        session.resolveBlockingTask(this.moduleName, "mining");
        await session.tryProceedSession();
    }
    async processPoWClientWebSocket(req, socket, head, remoteIp) {
        let sessionId;
        let clientVersion;
        try {
    ...
    getPoWServerForSession(session, create = false) {
...
//# sourceMappingURL=PoWModule.js.map