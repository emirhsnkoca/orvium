...
import { EthClaimNotificationClient } from './EthClaimNotificationClient.js';
import { clearInterval } from 'timers';
export var ClaimTxStatus;
...
export class EthClaimManager {
    ...
    lastClaimTxIdx = 1;
    lastProcessedClaimTxIdx = 0;
    lastConfirmedClaimTxIdx = 0;
    lastClaimNotification;
    async initialize() {
        if (this.initialized)
            return;
    ...
    async processClaimNotificationWebSocket(req, ws, remoteIp) {
        let sessionId;
        try {
    ...
    getLastProcessedClaimIdx() {
        return this.lastProcessedClaimTxIdx;
    }
    updateClaimStatus(claimInfo) {
        if (claimInfo.claim.claimStatus === ClaimTxStatus.CONFIRMED) {
            ...
            moduleManager.getModule("faucet-outflow")?.updateState(null, BigInt(claimInfo.claim.txFee));
    ...
    async createSessionClaim(sessionData, userInput) {
    ...
    async processQueue() {
        ...
        try {
            ...
            while (Object.keys(this.pendingTxQueue).length < faucetConfig.ethMaxPending && this.claimTxQueue.length > 0) {
                if (faucetConfig.ethQueueNoFunds && (!walletState.ready ||
    ...
    async processQueueTx(claimTx) {
        ...
        if (walletState.balance - BigInt(faucetConfig.spareFundsAmount) < BigInt(claimTx.amount) ||
            walletState.nativeBalance <= BigInt(faucetConfig.ethTxGasLimit) * BigInt(faucetConfig.ethTxMaxFee)) {
            claimTx.claim.claimStatus = ClaimTxStatus.FAILED;
    ...
    awaitTxReceipt(claimTx, txPromise) {
...
//# sourceMappingURL=EthClaimManager.js.map