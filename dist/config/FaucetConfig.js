...
export function setAppBasePath(basePath) {
    internalBasePath = basePath;
}
export function loadFaucetConfig(loadDefaultsOnly) {
    ...
    if (config) {
        if (!config.faucetSecret)
            config.faucetSecret = randomBytes(40).toString("hex");
...
export function resolveRelativePath(inputPath, customBasePath) {
