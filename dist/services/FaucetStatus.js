...
export var FaucetStatusLevel;
(function (FaucetStatusLevel) {
    FaucetStatusLevel["INFO"] = "info";
    FaucetStatusLevel["WARNING"] = "warn";
    FaucetStatusLevel["ERROR"] = "error";
})(FaucetStatusLevel || (FaucetStatusLevel = {}));
export class FaucetStatus {
    ...
    setFaucetStatus(key, statusText, statusLevel, prio) {
        if (statusText) {
    ...
    getCachedValue(key, timeout, getter) {
        let cachedValue;
    ...
    getFaucetStatus(clientVersion, session) {
        let statusList = [];
            ...
            if (restriction) {
                restriction.messages.forEach((message) => {
                    if (!message.notify)
                        return;
                    addStatus({
                        level: (typeof message.notify === "string" ? message.notify : FaucetStatusLevel.WARNING),
                        prio: 20,
                        text: message.text,
                    });
                });
